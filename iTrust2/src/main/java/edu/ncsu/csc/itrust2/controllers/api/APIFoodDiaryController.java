...
//    }
     ...
     *
     * @param entryF
     *            The DiaryEntryForm to be parsed into an
     *            DiaryEntry and stored
     * @return The parsed and validated Diary Entry created from the Form
     *         provided, HttpStatus.CONFLICT if a entry already exists with
     *         the ID of the provided entry, or HttpStatus.BAD_REQUEST if
     *         another error occurred while parsing or saving the entry
     *         provided
     */
    ...
    public ResponseEntity createDiaryEntry ( @RequestBody final DiaryEntryForm entryF ) {
        try {
            ...
            return new ResponseEntity( errorResponse( "Error occured while validating or saving " + entryF.toString()
